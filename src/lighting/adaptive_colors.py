...
class AdaptiveLightingManager:
    ...
    def analyze_frame(self, frame: np.ndarray) -> float:
        ...
        try:
            ...
            self.logger.error(
                "AdaptiveLighting",
                f"Błąd podczas analizy jasności: {str(e)}",
                log_type="LIGHTING"
            )
    ...
    def update_colors(self, frame_brightness: float) -> Tuple[Tuple[int, int, int], Tuple[int, int, int]]:
        """
        ...
        target_bg_value = self.min_brightness + frame_brightness * (self.max_brightness - self.min_brightness)
        ...
        self.target_bg_brightness = self.target_bg_brightness * (
                1 - adaptation_factor) + target_bg_value * adaptation_factor

        # Ograniczenie do zakresu
        self.target_bg_brightness = max(self.min_brightness, min(self.max_brightness, self.target_bg_brightness))
